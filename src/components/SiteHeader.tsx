    ...
    <>
      ...
      >
        <div className="container-prose flex h-14 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-7">
            {NAV.map(({ to, label }) => (
              <Link key={to} to={to} className="text-xs font-semibold uppercase tracking-[0.18em] text-background/45 hover:text-background transition-colors duration-200 py-1" activeProps={{ className: "text-gold" }}>
                {label}
              </Link>
            ))}
          </nav>
          <div className="flex items-center gap-4">
            <a href="https://purdue.ca1.qualtrics.com/jfe/form/SV_1G5FfwJUc1cGJ2m" target="_blank" rel="noopener noreferrer" className="hidden md:inline-flex items-center px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.2em] bg-gold text-ink hover:bg-gold-mid transition-colors duration-200">Apply</a>
      ...
      <div className="fixed inset-0 z-[60] bg-ink flex flex-col transition-opacity duration-300" style={{ opacity: open ? 1 : 0, pointerEvents: open ? "auto" : "none" }}>
        <div className="container-prose flex h-14 items-center justify-between border-b border-white/8 shrink-0">
          <span className="font-mono text-xs uppercase tracking-[0.28em] text-background/60">SMIF</span>
        ...
        <div className="container-prose pb-6 shrink-0"><span className="text-xs uppercase tracking-[0.3em] text-background/20 font-mono">Daniels School of Business · Est. 2009</span></div>
      </div>
    </>
  );
}
