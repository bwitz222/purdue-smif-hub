...
function HoldingsPage() {
  ...
  return (
    <>
      <section className="border-b border-border bg-secondary/30"><div className="container-prose py-20"><span className="rule-gold mb-5 block" /><span className="text-xs font-semibold uppercase tracking-[0.32em] text-gold-deep block mb-4">Portfolio Holdings</span><h1 className="font-display font-bold text-ink" style={{ fontSize: "clamp(2.5rem, 6vw, 5rem)" }}>Current portfolio.</h1><p className="mt-5 max-w-2xl text-muted-foreground leading-relaxed">A live snapshot of every position held by the Purdue Student Managed Investment Fund, with cost basis, returns, and portfolio weighting.</p><div className="mt-6 inline-flex items-center gap-2 text-xs font-mono text-muted-foreground">{isFetching ? (<><RefreshCw className="h-3 w-3 animate-spin text-gold" />Refreshing live prices…</>) : quoteData?.cachedAt ? (<><span className="inline-block h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />Live via Alpha Vantage · updated {new Date(quoteData.cachedAt).toLocaleTimeString()} · refreshing every 60s</>) : (<><span className="inline-block h-1.5 w-1.5 rounded-full bg-muted-foreground/40" />Showing last reported snapshot</>)}</div></div></section>
      <section className="container-prose py-14 space-y-10">
        ...
        <div className="grid gap-px bg-border md:grid-cols-3"><div className="md:col-span-2 bg-card border border-border p-6"><div className="text-xs uppercase tracking-[0.22em] text-muted-foreground mb-5">Sector Allocation</div><div className="space-y-2.5">{sectorBreakdown.map(([s, pct]) => (<div key={s} className="flex items-center gap-3 text-sm"><span className="w-44 flex-shrink-0 truncate text-xs text-foreground/80">{s}</span><div className="flex-1 h-1.5 bg-muted"><div className="h-full bg-gradient-gold transition-all duration-500" style={{ width: `${maxSectorPct > 0 ? (pct / maxSectorPct) * 100 : 0}%` }} /></div><span className="w-12 text-right font-mono text-xs text-muted-foreground">{pct.toFixed(1)}%</span></div>))}</div></div><div className="bg-card border border-border p-6 flex flex-col gap-6"><div><div className="text-xs uppercase tracking-[0.22em] text-muted-foreground mb-2">Weighted Beta</div><div className="font-display text-4xl font-bold text-ink">{portfolioSummary.weightedBeta.toFixed(2)}</div><div className="text-xs text-muted-foreground mt-1 font-mono">vs. 1.00 S&amp;P 500</div></div><div className="border-t border-border pt-5"><div className="text-xs uppercase tracking-[0.22em] text-muted-foreground mb-2">Invested Capital</div><div className="font-display text-2xl font-bold text-ink">{fmtUSD(portfolioSummary.investedCapital, { maximumFractionDigits: 0 })}</div></div></div></div>
        <div className="flex flex-wrap items-center gap-2"><span className="text-xs uppercase tracking-[0.22em] text-muted-foreground mr-1">Sector</span>{sectors.map((s) => (<button key={s} onClick={() => setSector(s)} className={`px-3 py-1.5 text-xs font-semibold uppercase tracking-wider border transition-colors duration-150 cursor-pointer ${sector === s ? "bg-ink text-background border-ink" : "bg-background text-foreground border-border hover:border-ink hover:bg-secondary"}`}>{s}</button>))}</div>
